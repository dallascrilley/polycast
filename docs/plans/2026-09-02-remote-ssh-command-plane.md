...
and iPad Shortcuts use Cherri's SSH action; Termux uses the existing `mac-exec`
...
Private connection state belongs in a local profile file. Its key file and host
are read while building the iPhone/iPad artifact and never enter `CommandDef`,
the public schema examples, or Git. The resulting build output is already
ignored, so it must be handled as credential-bearing material.
...
- A `none` or `text` command with `x.remote.profile` can emit a Cherri SSH
  Shortcut from a private profile and a Termux shortcut script.
